...
class ActionSpace:
    def __init__(self, n: int):
        """
        Action space with n possiable actions an agent can select.
        """
...
class ObservationSpace:
    def __init__(self, n: int, shape: Tuple[int]):
        ...
        for i in range(n):
            ...
            lower, upper = upper, upper + 10 # Incriment as we go to make each state easily identifiable
...
class EnvTest:
    ...
    There is a fixed reward that is recieved by the agent when it moves into a new state recorded in the
    self.rewards property. When the agent takes action X and moves from state S to X, it recieves reward
    ...
    def step(self, action: int) -> None:
        ...
        self.num_iters += 1 # Incriment the number of iterations taken within the current episode
        ...
        reward *= self.rewards[self.cur_state] # Record the reward recieved by moving to the new state
